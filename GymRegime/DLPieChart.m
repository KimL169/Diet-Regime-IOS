...
- (void)renderInLayer:(DLPieChart *)layerHostingView dataArray:(NSMutableArray*)dataArray
{
    ...
    //set the colors to the protein colors.
    [self.DLColorsArray addObject:[UIColor colorWithRed: 0.41 green: 1 blue: 0.114 alpha: 1]];
    [self.DLColorsArray addObject:[UIColor colorWithRed: 0.343 green: 0.343 blue: 1 alpha: 1]];
    [self.DLColorsArray addObject:[UIColor colorWithRed: 1 green: 0.114 blue: 0.114 alpha: 1]];
    ...
    //make sure the legend only gets drawn once.
    if (self.isChartLoaded == NO) {
        //draw the chartlegend
        [self drawLegends:layerHostingView dataArray:dataArray];
        //set ischartloaded to yes.
...
-(void)drawLegends:(DLPieChart *)layerHostingView dataArray:(NSMutableArray*)dataArray
{
    ...
    //create a title array to hold the titles.
    NSArray *titleArray = @[@"protein", @"carbs", @"fat"];
    for (int i=0;i<dataArray.count;i++)
    {
        ...
        UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake(40, y, legendWidth, 25)];
        lbl.text = [NSString stringWithFormat:@"%@",[titleArray objectAtIndex:i]];
